...
(function() {
	...
	// Register a plugin named "stylesheetparser-inline".
	CKEDITOR.plugins.add( 'stylesheetparser-inline', {
		init: function( editor ) {
			...
			editor.once( 'stylesSet', function( evt ) {
				...
				var $iframe = $('<iframe>').attr('id', 'inline-editor-styles').hide();
				$('body').append($iframe);
				$iframe.contents().find("head").append(
					$('<link rel="stylesheet" type="text/css" href="'+editor.config.contentsCss+'">')
				);
				...
				editor.once( 'contentDom', function() {
					editor.getStylesSet( function( definitions ) {
						...
						cachedDefinitions = definitions.concat( LoadStylesCSS( $iframe.contents()[0], skipSelectors, validSelectors ) );
